...
async def get_portfolio(db=Depends(get_db)):
    ...
    DB-FIRST: Reads directly from database, enriched with live prices.
    """
    ...
    try:
        # Get trader state from DB
        trader_state = await TraderStateRepository.get_state()
        if not trader_state:
            # Return defaults if no state
            return {
        ...
        try:
            ws = get_binance_ws()
            live_prices = ws.get_all_prices() if ws else {}
        except:
            pass
        
        # Fallback to REST API if WebSocket prices empty
        if not live_prices and positions:
            try:
                binance = get_binance()
                for pos in positions:
                    price = binance.get_price(pos.symbol)
                    if price:
                        live_prices[pos.symbol] = price
            except Exception as e:
                logger.debug(f"Failed to get REST prices: {e}")
        
        # Update positions with live prices
        for pos in positions:
            pos.current_price = live_prices.get(pos.symbol, pos.entry_price)
        
        # Calculate totals
        positions_value = sum(p.size_usdt for p in positions)
        unrealized_pnl = sum(p.unrealized_pnl for p in positions)
        total_equity = trader_state.current_capital + unrealized_pnl
        total_pnl = trader_state.total_pnl + unrealized_pnl
        
        return {
            "total_equity": total_equity,
            "initial_capital": trader_state.starting_capital,
            "cash": trader_state.current_capital,
            "positions_value": positions_value,
            "unrealized_pnl": unrealized_pnl,
            "realized_pnl": trader_state.total_pnl,
            "total_pnl": total_pnl,
            "pnl_percent": (total_pnl / trader_state.starting_capital * 100) if trader_state.starting_capital > 0 else 0,
            "open_positions": len(positions),
            "total_trades": trader_state.total_trades,
            "winning_trades": trader_state.winning_trades,
            "losing_trades": trader_state.losing_trades,
            "win_rate": trader_state.win_rate,
            "total_volume": 0.0,
            "total_fees": trader_state.total_fees,
            "total_spread": trader_state.total_spread,
            "total_slippage": trader_state.total_slippage,
            "total_trading_costs": trader_state.total_trading_costs,
            "var_95": None,
            "max_drawdown": 0.0,
        }
    except Exception as e:
        logger.error(f"Failed to get portfolio from DB: {e}")
...
async def get_positions(status: str = "open", db=Depends(get_db)):
    ...
    try:
        if status.lower() == "open":
            ...
            # Build response with live prices
            result = []
            for pos in positions:
                ...
                result.append({
                    ...
                    'margin_risk_pct': pos.margin_risk_pct,
                    'conviction': pos.conviction,
                    'reason': pos.reasoning,
                })
            return result
        
        # For closed positions, query from DB
        if status.lower() == "closed":
            positions = await PositionRepository.get_closed_positions(limit=100)
            return [
                {
                    'id': pos.id,
                    'symbol': pos.symbol,
...
@app.get("/api/positions-legacy", tags=["Positions"])
async def get_positions_legacy(status: str = "open", db=Depends(get_db)):
    """
    LEGACY: Get positions from in-memory state.
...
async def get_trades(limit: int = 100, transaction_type: str = None, db=Depends(get_db)):
    """
    Get recent trades from paper_trades.
    
    Args:
        limit: Maximum number of trades to return
        transaction_type: Optional filter by type (OPEN, CLOSE, EXTEND, REDUCE)
    """
    from sqlalchemy import text
    ...
    try:
        ...
        query = """
            SELECT id, position_id, symbol, direction, entry_price, exit_price,
                   ...
                   COALESCE(transaction_type, 'CLOSE') as transaction_type,
                   COALESCE(position_size_before, 0) as position_size_before,
                   COALESCE(position_size_after, 0) as position_size_after
            FROM paper_trades
        """
        params = {"limit": limit}
        ...
        query += " ORDER BY COALESCE(exit_time, created_at) DESC LIMIT :limit"
        ...
        return [{
            ...
            'size_usdt': float(t['size_usdt']) if t['size_usdt'] else 0,
            'leverage': t['leverage'] or 1,
            'pnl_usdt': float(t['pnl_usdt']) if t['pnl_usdt'] else 0,
            ...
            'position_size_before': float(t['position_size_before']) if t['position_size_before'] else 0,
            'position_size_after': float(t['position_size_after']) if t['position_size_after'] else 0,
        } for t in trades]
    except Exception as e:
...
@app.get("/api/transactions", tags=["Trades"])
async def get_transactions(limit: int = 100, transaction_type: str = None, db=Depends(get_db)):
    """
    Get all position transactions from the in-memory tracker.
    
    Returns all transaction types: OPEN, CLOSE, EXTEND, REDUCE.
    Falls back to database if in-memory data not available.
    """
    from sqlalchemy import text
    
    try:
        # First try to get from Portfolio Manager's in-memory tracker
        try:
            from portfolio_manager import get_portfolio_manager
            manager = get_portfolio_manager()
            
            if manager and manager.positions and manager.positions.transactions:
                transactions = manager.positions.transactions
                ...
                return [t.to_dict() for t in reversed(transactions[-limit:])]
        except Exception as e:
            logger.debug(f"Could not get transactions from Portfolio Manager: {e}")
        
        # Fallback to database
        query = """
            SELECT id, position_id, symbol, direction, transaction_type,
                   price, quantity, size_usdt, leverage,
                   position_size_before, position_size_after,
                   avg_entry_before, avg_entry_after,
                   realized_pnl, realized_pnl_pct,
                   fee, spread, slippage,
                   reason, conviction, created_at
        ...
        return [{
            ...
            'position_size_before': float(t['position_size_before']) if t['position_size_before'] else 0,
            'position_size_after': float(t['position_size_after']) if t['position_size_after'] else 0,
            'avg_entry_before': float(t['avg_entry_before']) if t['avg_entry_before'] else 0,
            'avg_entry_after': float(t['avg_entry_after']) if t['avg_entry_after'] else 0,
            'realized_pnl': float(t['realized_pnl']) if t['realized_pnl'] else 0,
            'realized_pnl_pct': float(t['realized_pnl_pct']) if t['realized_pnl_pct'] else 0,
            'fee': float(t['fee']) if t['fee'] else 0,
            'spread': float(t['spread']) if t['spread'] else 0,
            'slippage': float(t['slippage']) if t['slippage'] else 0,
            'reason': t['reason'],
            'conviction': t['conviction'] or 0,
...
@app.get("/api/transactions/summary", tags=["Trades"])
async def get_transaction_summary():
    """
    Get summary of transactions by type.
    
    Returns counts and totals for each transaction type.
    """
    try:
        from portfolio_manager import get_portfolio_manager
        manager = get_portfolio_manager()
        
        if manager and manager.positions:
            stats = manager.positions.get_statistics()
            return {
                "transactions": stats.get("transactions", {}),
                "deployment": stats.get("deployment", {}),
            }
        
        return {
            "transactions": {"total": 0, "open": 0, "close": 0, "extend": 0, "reduce": 0},
            "deployment": {"total_deployed": 0, "deployment_percent": 0, "below_minimum": False},
        }
    except Exception as e:
...
async def get_margin_health(db=Depends(get_db)):
    ...
    try:
        ...
        for symbol, pos in executor.positions.items():
            ...
            # Calculate margin for this position (size_usdt is notional, divide by leverage)
...
# ═══════════════════════════════════════════════════════════════════════════════
# LOGBOOK ENDPOINTS (Portfolio Manager)
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/api/logbook", tags=["Logbook"])
async def get_logbook_entries(limit: int = 20):
    """
    Get recent logbook entries with Grok's analysis.
    
    Each entry contains:
    - Human-readable analysis text from Grok
    - Position decisions (keep/close)
    - New opportunities
    - Portfolio state at that cycle
    """
    try:
        from portfolio_manager import get_logbook
        logbook = get_logbook()
        
        entries = logbook.get_latest(limit)
        return [e.to_dict() for e in entries]
    except Exception as e:
        logger.warning(f"Failed to get logbook entries: {e}")
        return []


@app.get("/api/logbook/summaries", tags=["Logbook"])
async def get_logbook_summaries(limit: int = 50):
    """Get logbook entry summaries for list views."""
    try:
        from portfolio_manager import get_logbook
        logbook = get_logbook()
        
        return logbook.get_summaries(limit)
    except Exception as e:
        logger.warning(f"Failed to get logbook summaries: {e}")
        return []


@app.get("/api/logbook/stats", tags=["Logbook"])
async def get_logbook_stats():
    ...
    try:
        ...
        return logbook.get_statistics()
    except Exception as e:
        logger.warning(f"Failed to get logbook stats: {e}")
        return {}


@app.get("/api/logbook/search", tags=["Logbook"])
async def search_logbook(query: str, limit: int = 20):
    """Search through logbook entries."""
    try:
        from portfolio_manager import get_logbook
        logbook = get_logbook()
        
        entries = logbook.search(query, limit)
        return [e.to_dict() for e in entries]
    except Exception as e:
        logger.warning(f"Failed to search logbook: {e}")
        return []


@app.get("/api/logbook/{entry_id}", tags=["Logbook"])
async def get_logbook_entry(entry_id: str):
    ...
    try:
        ...
        logger.warning(f"Failed to get logbook entry: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/portfolio-manager/status", tags=["Portfolio Manager"])
async def get_portfolio_manager_status():
    """
    Get Portfolio Manager status.
    
    DB-FIRST: Reads positions and state from database.
    """
    from repositories import PositionRepository, TraderStateRepository
    from integrations.binance_ws import get_binance_ws
    from integrations.binance import get_binance
    ...
    try:
        ...
        unrealized_pnl = sum(p.unrealized_pnl for p in positions)
        total_equity = (trader_state.current_capital if trader_state else 100000) + unrealized_pnl
        
        # Build position data for response
        position_data = [
            {
                "id": p.id,
                "symbol": p.symbol,
        ...
        return {
            "running": True,
            "phase": "idle",
            "cycle_count": trader_state.total_cycles if trader_state else 0,
            "cycle_interval_seconds": 600,
            "mode": "paper",
            "db_first": True,  # Indicator that this is DB-first architecture
            "portfolio": {
                "positions": position_data,
                "statistics": {
                    "starting_capital": trader_state.starting_capital if trader_state else 100000,
                    "current_capital": trader_state.current_capital if trader_state else 100000,
                    "total_equity": total_equity,
                    "unrealized_pnl": unrealized_pnl,
                    "realized_pnl": trader_state.total_pnl if trader_state else 0,
                    "total_pnl": (trader_state.total_pnl if trader_state else 0) + unrealized_pnl,
                    "open_positions": len(positions),
                    "max_positions": 50,
                    "available_slots": 50 - len(positions),
                    "total_trades": trader_state.total_trades if trader_state else 0,
                    "winning_trades": trader_state.winning_trades if trader_state else 0,
                    "losing_trades": trader_state.losing_trades if trader_state else 0,
                    "win_rate": trader_state.win_rate if trader_state else 0,
                    "total_fees": trader_state.total_fees if trader_state else 0,
                    "total_spread": trader_state.total_spread if trader_state else 0,
                    "total_slippage": trader_state.total_slippage if trader_state else 0,
                },
            },
            "logbook": {"total_entries": 0, "entries": []},
            "snapshot_service": {"running": True},
            "db_restored": True,
        }
    except Exception as e:
        logger.warning(f"Failed to get portfolio manager status from DB: {e}")
        return {"error": str(e), "db_first": True}


@app.post("/api/portfolio-manager/cycle", tags=["Portfolio Manager"])
async def trigger_cycle():
    ...
    # Run cycle in background task to avoid blocking
    asyncio.create_task(manager.run_cycle())
    
    return {
        "status": "triggered",
        "message": "Cycle started",
        "cycle_number": manager._cycle_count + 1
    }
...
async def websocket_equity_stream(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            try:
                # Try Portfolio Manager first for real-time data
                equity_data = None
                try:
                    from portfolio_manager import get_portfolio_manager
                    from integrations.binance_ws import get_binance_ws
                    ...
                    manager = get_portfolio_manager()
                    if manager and manager.positions:
                        # Update prices from Binance WebSocket
                        ws = get_binance_ws()
                        live_prices = ws.get_all_prices()
                        
                        # If WebSocket prices are empty, try REST API
                        if not live_prices and manager.positions.positions:
                            try:
                                binance = get_binance()
                                for symbol in manager.positions.positions.keys():
                                    price = binance.get_price(symbol)
                                    if price:
                                        live_prices[symbol] = price
                            except Exception as e:
                                pass  # Silently fail in WebSocket loop
                        
                        if live_prices:
                            manager.positions.update_prices(live_prices)
                        
                        pos_tracker = manager.positions
                        equity_data = {
                            "timestamp": datetime.utcnow().isoformat(),
                            "total_equity": pos_tracker.total_equity,
                            "cash": pos_tracker.capital,
                            "positions_value": sum(p.size_usdt for p in pos_tracker.positions.values()),
                            "unrealized_pnl": pos_tracker.total_unrealized_pnl,
                            "position_details": [
                                {
                                    "symbol": p.symbol,
                                    "direction": p.direction,
                                ...
                                for p in pos_tracker.positions.values()
                            ]
                        }
                except Exception as e:
                    logger.debug(f"Portfolio Manager equity not available: {e}")
                
                # Fallback to legacy tracker
                if equity_data is None:
                    latest = tracker.get_latest()
                    if latest:
                        equity_data = latest.to_dict()
                    else:
                        equity_data = {
                            "timestamp": datetime.utcnow().isoformat(),
                            "total_equity": 100000,
        ...
        logger.debug("Equity WebSocket disconnected")
    except Exception as e:
        if str(e):  # Only log if there's an actual error message
            logger.warning(f"Equity WebSocket error: {type(e).__name__}: {e}")
    finally:
...
async def reset_paper_trades(db=Depends(get_db)):
    ...
    try:
        # DB-FIRST ARCHITECTURE: Database is the single source of truth.
        # No in-memory state to reset - all components read directly from DB.
        logger.warning("RESET: Starting database reset (DB-First architecture)")
        
        # Use TRUNCATE CASCADE for faster deletion with foreign key handling
        tables_to_clear = [
            "paper_trades",           
            "paper_positions",        
            "predictions",            
        ...
        for table in tables_to_clear:
            try:
                ...
                deleted_counts[table] = count_result.scalar() or 0
            except:
                deleted_counts[table] = 0
        
        # Use TRUNCATE CASCADE for all tables at once (faster, handles FK)
        try:
            ...
            logger.info(f"Truncated all tables: {tables_to_clear}")
        except Exception as truncate_error:
            logger.warning(f"TRUNCATE failed, falling back to DELETE: {truncate_error}")
            # Fallback: DELETE in correct order (children first)
            for table in tables_to_clear:
                try:
        ...
        # Reset trader_state to initial values (UPDATE, not DELETE - it's a singleton)
        try:
            await db.execute(text("""
                ...
                WHERE id = 'main'
            """))
            logger.info("Reset trader_state to initial values")
        except Exception as state_error:
            logger.warning(f"Could not reset trader_state: {state_error}")
        
        # Reset account_state if exists (optional table)
        try:
            await db.execute(text("""
            ...
            """))
            logger.info("Reset account_state to initial values")
        except Exception as account_error:
            # This table may not exist or have different columns - that's OK
            logger.debug(f"account_state reset skipped: {account_error}")
        
        # CRITICAL: Commit the transaction BEFORE any post-reset actions
        await db.commit()
        logger.warning("DATABASE RESET COMPLETE - Transaction committed")
        
        # Verify reset worked
        try:
            verify_result = await db.execute(text("SELECT current_capital, total_trades FROM trader_state WHERE id = 'main'"))
            row = verify_result.fetchone()
            if row:
                logger.info(f"Verified: capital=${row[0]}, trades={row[1]}")
        except Exception as e:
            logger.warning(f"Could not verify reset: {e}")
        ...
        # DB-FIRST: All data was in database, now cleared
        total_deleted = sum(deleted_counts.values())
        
        logger.warning(f"RESET COMPLETE: Database cleared, {total_deleted} rows deleted")
        
        return {
            "status": "success",
            "message": "All paper trades and positions have been reset (DB-First architecture).",
