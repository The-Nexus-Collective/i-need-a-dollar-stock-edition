...
public interface TradeRepository extends JpaRepository<Trade, String> {
     ...
     * Calculate total PnL
     */
    @Query("SELECT COALESCE(SUM(t.pnlUsdt), 0) FROM Trade t")
    BigDecimal calculateTotalPnl();

    /**
     * Calculate total PnL since timestamp
     */
    @Query("SELECT COALESCE(SUM(t.pnlUsdt), 0) FROM Trade t WHERE t.exitTime >= :since")
    BigDecimal calculatePnlSince(@Param("since") Instant since);

    /**
     * Count winning trades
     */
    @Query("SELECT COUNT(t) FROM Trade t WHERE t.pnlUsdt > 0")
