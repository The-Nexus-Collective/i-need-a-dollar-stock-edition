...
public class MarginMonitorService {
    ...
    private MarginCallInfo executeMarginCall(Position position, BigDecimal currentPrice, 
        ...
        try {
            ...
            // Create risk event for the margin call
            riskService.createMarginCallEvent(
                    position.getId(),
                    position.getSymbol(),
    ...
    /**
     * Create a warning event for a position approaching liquidation.
     */
    private void createWarningEvent(Position position, BigDecimal distanceToLiq, 
                                     ...
                                     String severity) {
        try {
            riskService.createMarginWarningEvent(
                    position.getId(),
                    position.getSymbol(),
                    position.getDirection(),
                    distanceToLiq,
                    "critical".equals(severity) ? criticalThreshold : warningThreshold,
                    currentPrice,
                    liquidationPrice,
                    BigDecimal.valueOf(position.getLeverage())
            );
        } catch (Exception e) {
            log.error("Failed to create margin warning event: {}", e.getMessage());
        }
    }
